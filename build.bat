...
"%programfiles(x86)%\MSBuild\14.0\Bin\MSBuild.exe" Nugety.sln /p:Configuration="%config%" /m /v:M /fl /flp:LogFile=msbuild.log;Verbosity=Normal /nr:false
REM call dotnet build --configuration %config%
if not "%errorlevel%"=="0" goto failure

REM Unit tests
rem cd Nugety\src\Tests\Nugety.Tests
